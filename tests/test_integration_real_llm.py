...
def temp_test_files():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create test files
        (temp_path / "test.txt").write_text("This is a test file for integration testing.")
        (temp_path / "data.json").write_text('{"name": "integration_test", "status": "active"}')
        (temp_path / "readme.md").write_text("# Test Directory\nThis directory contains test files.")
...
class TestRealLLMIntegration:
    ...
    async def test_anthropic_with_mock_filesystem(self, real_anthropic_config, temp_test_files):
        ...
        try:
            ...
            prompt = f"Please list the files in {temp_test_files} and tell me what you find."
            result = await agent.run(prompt)
            
            # Verify the agent completed the task
            assert result.success
            assert len(result.steps) > 0
            assert "test.txt" in result.final_answer.lower() or "test file" in result.final_answer.lower()
