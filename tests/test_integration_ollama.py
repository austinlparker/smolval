...
def ollama_available():
    ...
    try:
        ...
        if "gemma3:1b-it-qat" not in model_names:
            pytest.skip("Required model gemma3:1b-it-qat not found in Ollama. Run: ollama pull gemma3:1b-it-qat")
...
def test_ollama_gemma_function_calling(ollama_config, ollama_available):
    ...
    async def run_test():
        ...
        try:
            ...
            messages = [
                LLMMessage(role="user", content="Use the read_file function to read /tmp/test.txt. You MUST use the tool_code format.")
            ]
            ...
            assert len(response.tool_calls) > 0, "Should generate at least one tool call"
            ...
            test_cases = [
                ("With assignment", """```tool_code
result = read_file(path="/tmp/test.txt")
```"""),
                ("Direct call", """```tool_code
read_file("/tmp/test.txt")
```"""),
                ("Named args", """```tool_code
...
```"""),
            ...
            is_gemma = llm_client.config.provider == "ollama" and "gemma" in llm_client.config.model.lower()
            print(f"Detected as Gemma model: {is_gemma}")
            assert is_gemma, "Should detect Gemma model format"
            
            print(f"✅ All Gemma function calling tests passed!")
...
@pytest.mark.integration  
@pytest.mark.slow
@pytest.mark.skip(reason="Ollama Gemma model tool calling needs tuning - tool name mismatch")
def test_ollama_agent_evaluation(ollama_config, ollama_available):
    ...
    async def run_test():
        ...
        try:
            ...
            expected_words = ["example", "domain", "website", "page", "fetch", "content", "http"]
            assert any(word in final_answer_lower for word in expected_words), \
