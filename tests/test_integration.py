...
def temp_test_directory(tmp_path: Path):
    ...
    return {"test_dir": test_dir, "test_file": test_file, "data_file": data_file}


class TestFileSystemIntegration:
    """Integration tests with real filesystem MCP server."""

    @pytest.mark.skipif(os.system("which npx") != 0, reason="NPM/npx not available")
    @pytest.mark.requires_npm
    async def test_file_read_integration(
        self, filesystem_integration_config, temp_test_directory
    ):
        ...
        try:
            ...
            assert (
                len(found_tools) >= 2
            ), f"Expected filesystem tools not found. Available: {tool_names}"
    ...
    @pytest.mark.skipif(os.system("which npx") != 0, reason="NPM/npx not available")
    @pytest.mark.requires_npm
    async def test_agent_with_filesystem_server(
        self, filesystem_integration_config, temp_test_directory
    ):
        ...
        try:
            ...
            mock_responses = [
                ...
                MockLLMResponse(
                    ...
                    tool_calls=[
                        ToolCall(
                            id="call_1", name="list_allowed_directories", arguments={}
                        )
                    ],
                ),
                # Second: Final answer
...
class TestFetchIntegration:
    ...
    def fetch_integration_config(self, mock_llm_config: LLMConfig) -> Config:
        ...
        return Config(
            mcp_servers=[
                MCPServerConfig(
                    name="fetch", command=["uvx", "mcp-server-fetch"], env={}
                )
            ],
            llm=mock_llm_config,
    ...
    @pytest.mark.skipif(os.system("which uvx") != 0, reason="uvx not available")
    async def test_fetch_web_content(self, fetch_integration_config: Config):
        ...
        try:
            ...
            assert (
                "fetch" in tool_names
            ), f"Fetch tool not found. Available: {tool_names}"

            # Test fetching a simple webpage (example.com is reliable)
            result = await mcp_manager.execute_tool(
                "fetch", {"url": "https://example.com", "max_length": 500}
            )
            ...
            result2 = await mcp_manager.execute_tool(
                "fetch",
                {"url": "https://httpbin.org/json", "max_length": 200, "raw": True},
            )
    ...
    async def test_agent_with_fetch_server(self, fetch_integration_config: Config):
        ...
        try:
            ...
            result = await agent.run(
                "Fetch the content of example.com and tell me what it's about"
            )
...
"""
        ...
        try:
                        ...
                        return MockLLMResponse(
                            ...
                            tool_calls=[
                                ToolCall(
                                    id="call_2",
                                    name="read_file",
                                    arguments={"path": str(test_dir / "example.txt")},
                                )
                            ],
                        )
                    else:
            ...
            assert (
                len(results["result"]["steps"]) >= 2
            )  # At least list and read operations
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "requires_npm: mark test as requiring NPM")
