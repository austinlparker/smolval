...
class LLMClient:
    ...
    def __init__(self, config: LLMConfig):
        ...
        try:
            if config.provider == "ollama":
                ...
                if config.base_url:
                    self.model.base_url = config.base_url  # type: ignore[attr-defined]
            else:
                self.model = llm.get_model(config.model)
        except Exception as e:
            raise ValueError(
                f"Model '{config.model}' not found. Make sure the appropriate llm plugin is installed. Error: {e}"
            ) from e
    ...
    async def chat(
        self, messages: list[LLMMessage], tools: list[MCPTool] | None = None
    ) -> LLMResponse:
        ...
        for msg in messages:
            if msg.role == "system":
                ...
                conversation_parts.append(
                    f"Tool result (ID: {msg.tool_call_id}): {msg.content}"
                )
        ...
        try:
            ...
            if usage:
                token_usage = {
                    ...
                    "total_tokens": getattr(usage, "input", 0)
        ...
        try:
            ...
            if raw_response is None and response.response_json:
        ...
        for match in matches:
            ...
            if args_str.strip():
                ...
                if arg_matches:
                    ...
                    if positional_matches and func_name in [
                        "read_file",
                        "write_file",
                        "list_directory",
                    ]:
