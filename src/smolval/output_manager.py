...
class OutputManager:
    ...
    def write_evaluation_results(
        self, result_data: dict[str, Any], eval_name: str | None = None
    ) -> dict[str, str]:
        ...
        if eval_name:
            safe_eval_name = "".join(
                c for c in eval_name if c.isalnum() or c in ("-", "_")
            ).strip()
    ...
    def write_batch_results(
        self, batch_data: dict[str, Any], batch_name: str | None = None
    ) -> dict[str, str]:
        ...
        if batch_name:
            safe_batch_name = "".join(
                c for c in batch_name if c.isalnum() or c in ("-", "_")
            ).strip()
    ...
    def write_comparison_results(
        self, comparison_data: dict[str, Any], comparison_name: str | None = None
    ) -> dict[str, str]:
        ...
        # Generate base filename
        if comparison_name:
            safe_comparison_name = "".join(
                c for c in comparison_name if c.isalnum() or c in ("-", "_")
            ).strip()
    ...
    def _prepare_clean_result_data(self, result_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if "result" in clean_data and "token_usage" in clean_data["result"]:
            clean_data["result"] = {
                k: v for k, v in clean_data["result"].items() if k != "token_usage"
            }
