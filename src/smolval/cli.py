...
@click.option('--config', '-c', default='config/example-anthropic.yaml',
              help='Configuration file path')
@click.option('--output-dir', help='Output directory for results')
@click.option('--run-name', help='Name for this evaluation run')
@click.pass_context
def eval(ctx: click.Context, prompt_file: str, config: str, output_dir: str, run_name: str) -> None:
...
async def _run_eval(prompt_file: str, config_path: str, output_dir: str, run_name: str, debug: bool = False) -> None:
    ...
    try:
        ...
        spinner_idx = 0
        current_step = 0
        max_steps = config.evaluation.max_iterations
...
@click.pass_context
def batch(ctx: click.Context, prompts_dir: str, config: str, output_dir: str, run_name: str, servers: str) -> None:
...
@click.pass_context
def compare(ctx: click.Context, baseline: str, test: str, prompts_dir: str, config: str, output_dir: str, run_name: str) -> None:
...
async def _run_compare(baseline: str, test: str, prompts_dir: str, config_path: str, output_dir: str, run_name: str) -> None:
    ...
    try:
        ...
        for server_name in [baseline, test]:
            ...
            try:
                ...
                if mcp_manager is not None:
                    try:
                        logger = logging.getLogger(__name__)
                        logger.debug("Starting compare MCP cleanup for %s...", server_name)
                        await asyncio.wait_for(mcp_manager.close(), timeout=5.0)
                        logger.debug("Compare MCP cleanup completed for %s", server_name)
                    except asyncio.TimeoutError:
                        logger = logging.getLogger(__name__)
                        logger.warning("Compare MCP cleanup timed out for %s - forcing exit", server_name)
                    except Exception as cleanup_error:
                        logger = logging.getLogger(__name__)
                        logger.warning("Error during MCP cleanup for %s: %s", server_name, cleanup_error)
        ...
        output_files = output_manager.write_comparison_results(comparison_data, comparison_run_name)
